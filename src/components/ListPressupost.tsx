...
export default function ListPressupost() {
    ...
    const handleBudgetClick = (budget: ServiceProps) => {
        const url = `/pressupostdetail?name=${budget.name}&email=${budget.email}&phone=${budget.phone}&date=${budget.date}&value=${budget.services.map((service: { value: any; }) => service.value).join(",")}&total=${budget.budgetTotal}`;
        navigate(url);
    };
                    ...
                    <Card key={index} className="mt-6 w-full bg-deep-purple-50 items-start" onClick={() => handleBudgetClick(item)} >
                        <CardBody className="w-[100%] flex flex-row justify-between align-top">
                            ...
                            <div className="flex flex-col items- justify-start">
                                ...
                                {item.services.map((service: { value: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | null | undefined; }, index: React.Key | null | undefined) => (
